...
/**
 * Release pipeline for local macOS build verification.
 * Runs: preflight → typecheck → test → build → package+sign+notarize → verify.
 *
 * Usage:
 *   pnpm release:mac                         (build + verify only, no publish)
 *   pnpm release:mac --compile-release-notes (compile fragments for the current version without publishing)
 *
 * Public publishing is handled by scripts/release-public.mjs.
 */
...
const compileNotes = args.includes('--compile-release-notes')
const compileNotesOnly = compileNotes && !publish && !bump
...
if (publish) {
  fail('pnpm release:mac --publish is disabled after the private/public split. Use: pnpm release:public -- --bump patch')
}
...
let apiKeyPath = ''
if (compileNotesOnly) {
  console.log('  ✔ Compile Release Notes only; skipping signing and notarization preflight')
} else {
  ...
  apiKeyPath = process.env.APPLE_API_KEY.replace('~', process.env.HOME)
...
// ─── Bundled content + Release Notes ────────────────────────────────────────

console.log('\n━━━ Bundled content ━━━')
try {
  if (publish || compileNotes) {
    const result = compileReleaseNotes({ root, version, force: compileNotes })
    console.log(`  ✔ Release Notes ${result.action} for v${version}`)
    if (result.archivedFiles.length > 0) {
      console.log(`  ✔ Archived ${result.archivedFiles.length} release-note fragment${result.archivedFiles.length === 1 ? '' : 's'}`)
    }
  } else {
    validateContent(root)
    console.log('  ✔ Content Markdown validated')
  }
  const { outFile } = generateContentManifest({ root })
...
if (compileNotesOnly) {
  console.log('\n━━━ Done ━━━')
  console.log(`  📝 Release Notes compiled for v${version}`)
  console.log('')
  process.exit(0)
}
...
Signed and notarized with Developer ID.`
  ...
  // Commit version bump and bundled release-note changes if applicable
  if (bump) {
    run(`git add package.json pnpm-lock.yaml content src/renderer/src/generated/content-manifest.ts`)
