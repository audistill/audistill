import { app, shell, BrowserWindow, nativeTheme, ipcMain, dialog, clipboard, powerMonitor } from 'electron'
...
// Isolate dev data from production: use a separate userData directory in dev mode
if (!app.isPackaged) {
...
function registerDatabaseHandlers(): void {
  ...
  ipcMain.handle('db:delete-episode', async (_event, id: string) => {
    await ingestPipeline.discardEpisodeBeforeDelete(id)
    db.deleteEpisode(id)
  })
  ...
  ipcMain.handle('db:delete-episodes', async (_event, ids: string[]) => {
    for (const id of ids) {
      await ingestPipeline.discardEpisodeBeforeDelete(id)
    }
...
function createWindow(): void {
  ...
  if (is.dev && process.env['ELECTRON_RENDERER_URL']) {
    ...
    if (process.env['AUDISTILL_PROTOTYPE'] === 'recording-panel') {
      rendererUrl.searchParams.set('prototype', 'recording-panel')
...
app.whenReady().then(() => {
  ...
  const recordingsDirectory = join(app.getPath('userData'), 'recordings')
  ...
  const captureBackend = !supportedMacOS
    ? null
    : useFakeRecording
      ? new DevelopmentFakeCaptureBackend({
          recordingsDirectory,
          allowFakeCapture: true,
          availableCapacityBytes: process.env.AUDISTILL_FAKE_CAPACITY_FILE
            ? () => Number(readFileSync(process.env.AUDISTILL_FAKE_CAPACITY_FILE!, 'utf8').trim())
            : undefined,
        })
      : new NativeRecordingCaptureBackend({
          helperPath: app.isPackaged
            ? join(process.resourcesPath, 'native', 'audistill-capture-helper')
            : join(__dirname, '../../build/native/audistill-capture-helper'),
          recordingsDirectory,
        })
  recordingCoordinator = captureBackend
    ? new RecordingSessionCoordinator({
        backend: captureBackend,
        handoff: new RecordedEpisodeHandoff({
          db,
          enqueue: (episodeId) => ingestPipeline.enqueueEpisode(episodeId),
        }),
      })
    : null
  ...
  powerMonitor.on('suspend', () => { void recordingCoordinator?.handleSystemSuspend().catch(() => {}) })
...
app.on('before-quit', (event) => {
  ...
  recordingQuitDecisionInProgress = true
  void handleRecordingQuit(recordingCoordinator).finally(() => {
    recordingQuitDecisionInProgress = false
  })
})

async function handleRecordingQuit(coordinator: RecordingSessionCoordinator): Promise<void> {
  let state = await coordinator.waitUntilSettled()
  if (state.phase === 'recording' || state.phase === 'paused' || state.phase === 'completion-required') {
    const window = BrowserWindow.getFocusedWindow() ?? BrowserWindow.getAllWindows()[0]
    const canProcess = state.phase !== 'completion-required' || state.completionIssue?.canProcess === true
    const buttons = canProcess
      ? ['Stop and process', 'Discard', 'Keep recording']
      : ['Discard', 'Keep AudiStill open']
    const keepOpenIndex = buttons.length - 1
    const result = await dialog.showMessageBox(window, {
      type: 'warning',
      title: 'Recording in progress',
      message: 'What should AudiStill do with the current recording?',
      detail: 'AudiStill will never leave the capture helper running after the app quits.',
      buttons,
      defaultId: 0,
      cancelId: keepOpenIndex,
      noLink: true,
    })
    if (result.response === keepOpenIndex) return
    try {
      if (canProcess && result.response === 0) await coordinator.stop()
  ...
  quittingAfterCaptureCleanup = true
  app.quit()
}
