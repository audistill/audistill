...
import { parseFfmpegError, preprocess, spawnRecordedPreprocessor } from './audio-preprocessor'
...
export class IngestPipeline {
  ...
  private activePreprocessors = new Map<string, ReturnType<typeof spawnRecordedPreprocessor>>()
  ...
  recoverOrphanedEpisodes(): void {
    ...
    for (const ep of allEpisodes) {
      if (ep.source_type === 'recorded') continue
  ...
  async discardEpisodeBeforeDelete(episodeId: string): Promise<void> {
    const episode = this.db.getEpisode(episodeId)
    if (episode?.source_type === 'recorded' && !episode.transcript) {
      await this.discardRecordedEpisodeAudio(episode)
    } else {
      this.terminateWorkerForEpisode(episodeId)
    }
  }
  ...
  terminateWorkerForEpisode(episodeId: string): void {
    ...
    const preprocessor = this.activePreprocessors.get(episodeId)
    if (preprocessor) {
      preprocessor.kill('SIGTERM')
      this.activePreprocessors.delete(episodeId)
    }
  ...
  registerIPC(): void {
    ...
    ipcMain.handle('ingest:cancel', async (_event, episodeId: string) => {
      ...
      if (episode.source_type === 'recorded' && (episode.status === 'queued' || episode.status === 'transcribing')) {
        await this.discardRecordedEpisodeAudio(episode)
        this.db.updateEpisode(episodeId, {
          status: 'cancelled',
          file_path: null,
          error_message: 'Captured audio was discarded before a Transcript was created.',
        })
        this.broadcastEpisodeUpdate(episodeId)
        return
      }
  ...
  private async transcribeEpisode(episodeId: string, episode: Episode): Promise<boolean> {
    ...
    const ownershipPath = currentEp.file_path || episode.file_path
    const isUrlEpisode = !!currentEp.source_url
    const isRecordedEpisode = currentEp.source_type === 'recorded'
    ...
    try {
      ...
      if (isRecordedEpisode) {
        const recording = readAndValidateRecordingManifest(ownershipPath)
        this.db.updateEpisode(episodeId, { duration_sec: Math.round(recording.manifest.activeDurationMs / 1000) })
        this.broadcastProgress(episodeId, 'preparing', 100)
        this.broadcastEpisodeUpdate(episodeId)
        transcript = await this.runRecordedTranscriptionWorker(
          ownershipPath,
          modelPath,
          episodeId,
          Math.round(recording.manifest.activeDurationMs / 1000 * 16_000),
        )
      } else {
        const pcmBuffer = await preprocess(ownershipPath)
        this.db.updateEpisode(episodeId, { duration_sec: Math.round(pcmBuffer.byteLength / 4 / 16000) })
        this.broadcastEpisodeUpdate(episodeId)
        transcript = await this.runTranscriptionWorker(pcmBuffer, modelPath, episodeId)
      }
      ...
      const message = err instanceof Error ? err.message : String(err)
      if (current?.source_type === 'recorded' && !current.transcript) {
        await this.discardRecordedEpisodeAudio(current)
        this.db.updateEpisode(episodeId, {
          status: 'error',
          file_path: null,
          error_message: `${message} Captured audio was removed and this Episode cannot be retried.`,
        })
      } else {
        this.db.updateEpisode(episodeId, { status: 'error', error_message: message })
      }
  ...
  private async discardRecordedEpisodeAudio(episode: Episode): Promise<void> {
    const worker = this.activeWorkers.get(episode.id)
    if (worker) await worker.terminate()
    this.activeWorkers.delete(episode.id)
    const preprocessor = this.activePreprocessors.get(episode.id)
    if (preprocessor && preprocessor.exitCode === null) {
      preprocessor.kill('SIGTERM')
      await once(preprocessor, 'close').catch(() => {})
    }
    this.activePreprocessors.delete(episode.id)
    this.queue = this.queue.filter((id) => id !== episode.id)
    if (!episode.file_path) return
    try { rmSync(dirname(episode.file_path), { recursive: true, force: true }) } catch { /* startup reconciliation retries cleanup */ }
  }
  ...
  private runRecordedTranscriptionWorker(
    manifestPath: string,
    modelPath: string,
    episodeId: string,
    totalSamples: number,
    ...
    return new Promise((resolve, reject) => {
      const worker = new Worker(join(__dirname, 'transcription-worker.js'))
      const preprocessor = spawnRecordedPreprocessor(manifestPath)
      this.activeWorkers.set(episodeId, worker)
      this.activePreprocessors.set(episodeId, preprocessor)
      preprocessor.stdin.end()
      preprocessor.stdout.pause()

      const transcriptSegments: { start: number; end: number; text: string }[] = []
      let settled = false
      let ready = false
      let preprocessorClosed = false
      let inFlightChunk: string | null = null
      let chunkSequence = 0
      ...
      const maybeEndInput = (): void => {
        if (!ready || !preprocessorClosed || inFlightChunk !== null || settled) return
        if (remainder.length !== 0) {
          fail(new Error('Recorded audio stream ended with an incomplete PCM sample'))
          return
        }
      ...
      const sendChunk = (chunk: Buffer): void => {
        ...
        remainder = bytes.subarray(alignedLength)
        if (alignedLength === 0) {
          preprocessor.stdout.resume()
          return
        }
        const audioBuffer = Uint8Array.from(bytes.subarray(0, alignedLength)).buffer
        const id = String(++chunkSequence)
        inFlightChunk = id
        worker.postMessage({ type: 'stream-chunk', id, audioBuffer }, [audioBuffer])
      }

      preprocessor.stdout.on('data', (chunk: Buffer) => {
        preprocessor.stdout.pause()
        sendChunk(chunk)
      })
      preprocessor.stderr.on('data', (chunk: Buffer) => { stderr += chunk.toString() })
      preprocessor.on('error', (error) => fail(new Error(`FFmpeg failed: ${error.message}`)))
      preprocessor.on('close', (code) => {
        if (settled) return
        if (code !== 0) {
          fail(new Error(parseFfmpegError(stderr)))
          return
        }
        preprocessorClosed = true
        maybeEndInput()
      })
      ...
      worker.on('message', (message: {
        ...
        switch (message.type) {
          case 'stream-ready':
            ...
            maybeEndInput()
            break
          case 'stream-chunk-accepted':
            if (message.id !== inFlightChunk) break
            inFlightChunk = null
            if (preprocessorClosed) maybeEndInput()
            else preprocessor.stdout.resume()
            break
          case 'progress':
            this.broadcastProgress(episodeId, 'transcribing', message.percent ?? 0)
            break
          case 'segment':
            if (message.text) transcriptSegments.push({ start: message.start ?? 0, end: message.end ?? 0, text: message.text })
            break
          case 'done':
            if (settled) break
            settled = true
            cleanup()
            worker.terminate()
            resolve(JSON.stringify(transcriptSegments))
            break
          case 'error':
            fail(new Error(message.message || 'Transcription failed'))
            break
        }
      })
      ...
      worker.postMessage({ type: 'stream-start', modelPath, totalSamples })
    })
  }
  ...
  async runSummarization(episodeId: string): Promise<void> {
    ...
    let content = ''
    try {
      const execution = await this.recipeService.executeRecipe(pipelineRecipe.id, episode.transcript, (token) => {
        content += token
      ...
      this.tabService.updateTabGeneratedContent(tabId, finalContent, {
        generated_at: new Date().toISOString(),
        generated_model: execution?.model ?? this.recipeService.resolveModelForRecipe(pipelineRecipe.id),
      })
  ...
  async regenerateTab(episodeId: string, tabId: string): Promise<void> {
    ...
    let content = ''
    try {
      const execution = await this.recipeService.executeRecipe(tab.recipe_id, episode.transcript, (token) => {
        content += token
        this.broadcastTabEvent('tab:stream-token', { episodeId, tabId, token })
      })
      const { title, summary } = this.parseRecipeOutput(content)
      ...
      this.tabService.updateTabGeneratedContent(tabId, finalContent, {
        generated_at: new Date().toISOString(),
        generated_model: execution?.model ?? this.recipeService.resolveModelForRecipe(tab.recipe_id),
      })
