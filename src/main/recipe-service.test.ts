...
describe('RecipeService - message assembly', () => {
  ...
  it('returns and sends the raw default model id used for execution', async () => {
    db.setSetting('openrouter_api_key', 'test-key')
    db.setSetting('model_quality', 'openai/gpt-4.1-mini')
    const recipe = service.getPipelineRecipe()!
    mockStreamResponse('content')

    const result = await service.executeRecipe(recipe.id, 'transcript', () => {})
  ...
  it('returns and sends a recipe model override when configured', async () => {
    db.setSetting('openrouter_api_key', 'test-key')
    db.setSetting('model_quality', 'openai/gpt-4.1-mini')
    const recipe = service.getPipelineRecipe()!
    service.updateRecipe(recipe.id, { model_override: 'anthropic/claude-sonnet-4' })
    mockStreamResponse('content')
