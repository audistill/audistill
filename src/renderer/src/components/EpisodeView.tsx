...
function ProcessingState({ episode }: { episode: Episode }): React.JSX.Element {
  ...
  const statusText =
    episode.status === 'recovery-pending'
      ? 'Recording recovery is available in the Recording Workspace.'
  ...
  const retryUnavailable = episode.source_type === 'recorded' && !episode.transcript && !episode.file_path
...
function CancelledState({ episode }: { episode: Episode }): React.JSX.Element {
  const restartUnavailable = episode.source_type === 'recorded' && !episode.transcript && !episode.file_path
    ...
    <div className="flex-1 flex flex-col items-center justify-center px-12">
      <div className="w-full max-w-md text-center">
        ...
        <p className="text-sm text-[var(--secondary)] mb-4">
          {restartUnavailable ? 'Transcription cancelled and captured audio discarded' : 'Transcription cancelled'}
        </p>
        {!restartUnavailable && (
          <button
