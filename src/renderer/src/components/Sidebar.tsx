...
export function Sidebar({ prototypeReadOnly = false }: { prototypeReadOnly?: boolean }): React.JSX.Element {
              ...
              <div className="absolute top-full right-0 mt-1 w-48 bg-[var(--surface)] rounded-lg shadow-[0_8px_24px_rgba(0,0,0,0.4)] py-1.5 z-50">
                {recordingAvailable ? (
                  <button
                    ...
                    onClick={async () => {
                      setAddMenuOpen(false)
                      await window.api.recordingSession.open()
                      openRecordingWorkspace()
                    }}
                  >
                    Record audio… <span className="float-right text-[var(--secondary)]">⇧⌘R</span>
                  </button>
                ) : (
                  <div className="px-3 py-1.5 text-[13px] text-[var(--secondary)] opacity-70" title="Recording Sessions require macOS 13 or later">
                    Record audio unavailable
                  </div>
                )}
                <button
                  className="w-full text-left px-3 py-1.5 text-[13px] text-[var(--text)] hover:bg-white/[0.08] transition-colors"
...
function EpisodeContextMenu({
  ...
  const isRecordedWithoutAudio = episode?.source_type === 'recorded' && !episode.transcript && !episode.file_path
  ...
  } else if (isTranscribing || (episode?.source_type === 'recorded' && episode.status === 'queued')) {
    actions.push({ key: 'cancel', label: 'Cancel Transcription', action: () => onCancel(episodeId) })
  }
  if (isCancelled && !isRecordedWithoutAudio) {
    actions.push({ key: 'restart', label: 'Restart', action: () => onRetry(episodeId) })
  }
  if (isError && !isRecordedWithoutAudio) {
