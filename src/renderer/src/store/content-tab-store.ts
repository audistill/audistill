...
interface ContentTabState {
  ...
  endStreaming: (tabId: string, tab?: ContentTab) => void
...
export const useContentTabStore = create<ContentTabState>((set, get) => ({
  ...
  deleteTab: (tabId: string) => {
    ...
    if (streamingTabId === tabId) return
  ...
  endStreaming: (tabId: string, tab?: ContentTab) => {
    const { tabs } = get()
    set({
      streamingTabId: null,
      snapshotContent: null,
      tabs: tab ? tabs.map((t) => (t.id === tabId ? tab : t)) : tabs,
    })
  },
