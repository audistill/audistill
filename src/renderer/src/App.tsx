...
import { RecordingPanelPrototype } from './prototypes/recording-panel/RecordingPanelPrototype'
import { RecordingPrototypeTabBar } from './prototypes/recording-panel/RecordingPrototypeTabBar'
...
function App(): React.JSX.Element {
  const recordingPrototypeEnabled =
    import.meta.env.DEV &&
    new URLSearchParams(window.location.search).get('prototype') === 'recording-panel'
  ...
  }, [recordingPrototypeEnabled, showToast, modelState])
  ...
  }, [hydrated, recordingPrototypeEnabled])
  ...
  useEffect(() => {
    ...
    const unsubEnd = window.api.onTabStreamEnd((data) => {
      useContentTabStore.getState().endStreaming(data.tabId, data.tab)
    })
  ...
  const restoreRecordingWorkspace = useAppStore((s) => s.restoreRecordingWorkspace)

  useEffect(() => {
    if (!hydrated || recordingPrototypeEnabled) return
    let mounted = true
    window.api.recordingSession.getState().then((state) => {
      if (mounted && state.phase !== 'idle') restoreRecordingWorkspace()
    }).catch(() => {})
    const unsubscribe = window.api.recordingSession.onStateChanged((state) => {
      if (state.phase !== 'idle' && !useAppStore.getState().recordingWorkspaceOpen) restoreRecordingWorkspace()
    })
    return () => {
      mounted = false
      unsubscribe()
    }
  }, [hydrated, recordingPrototypeEnabled, restoreRecordingWorkspace])
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent): void => {
      ...
      if (e.metaKey && e.shiftKey && e.key.toLowerCase() === 'r') {
        e.preventDefault()
        window.api.recordingSession.open()
          .then(() => openRecordingWorkspace())
          .catch(() => {})
      }
    }
    window.addEventListener('keydown', handleKeyDown)
    return () => window.removeEventListener('keydown', handleKeyDown)
  }, [toggleLeftSidebar, toggleRightSidebar, toggleTranscriptPanel, openRecordingWorkspace])
  ...
  }, [recordingPrototypeEnabled, modelState])
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      >
        <div
          className={recordingPrototypeEnabled ? 'pointer-events-none' : ''}
          aria-hidden={recordingPrototypeEnabled || undefined}
        ...
        <div className="flex-1">
          {recordingPrototypeEnabled ? <RecordingPrototypeTabBar /> : <TabBar />}
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          <div
            ...
            className={`h-full ${recordingPrototypeEnabled ? 'pointer-events-none select-none opacity-60' : ''}`}
            aria-hidden={recordingPrototypeEnabled || undefined}
            inert={recordingPrototypeEnabled || undefined}
          >
            <Sidebar prototypeReadOnly={recordingPrototypeEnabled} />
          </div>
        </div>
        {leftSidebarOpen && (
          <ResizeHandle
        ...
        {recordingPrototypeEnabled ? <RecordingPanelPrototype /> : <ContentPane />}
        {rightSidebarOpen && (
          <ResizeHandle
            side="right"
        ...
        >
          <div
            ...
            className={`h-full bg-[var(--bg)] border-l border-[var(--surface)] flex flex-col overflow-hidden ${recordingPrototypeEnabled ? 'pointer-events-none select-none opacity-60' : ''}`}
            aria-hidden={recordingPrototypeEnabled || undefined}
            inert={recordingPrototypeEnabled || undefined}
          >
            <ChatSidebar prototypeReadOnly={recordingPrototypeEnabled} />
          </div>
        </div>
      </div>

      <DropOverlay visible={!recordingPrototypeEnabled && dropActive} />
      {!recordingPrototypeEnabled && droppedUrl && (
        <UrlImportPopover
