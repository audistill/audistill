...
const api = {
  ...
  starEpisode: (id: string): Promise<void> => ipcRenderer.invoke('db:star-episode', id),
  unstarEpisode: (id: string): Promise<void> => ipcRenderer.invoke('db:unstar-episode', id),
  ...
  // Recording Sessions
  recordingSession: {
    isAvailable: (): Promise<boolean> => ipcRenderer.invoke('recording-session:is-available'),
    getState: (): Promise<RecordingSessionState> => ipcRenderer.invoke('recording-session:get-state'),
    recover: (): Promise<StopRecordingSessionResult> => ipcRenderer.invoke('recording-session:recover'),
    discardRecovery: (): Promise<RecordingSessionState> => ipcRenderer.invoke('recording-session:discard-recovery'),
    open: (): Promise<RecordingSessionState> => ipcRenderer.invoke('recording-session:open'),
    refreshSources: (): Promise<RecordingSessionState> => ipcRenderer.invoke('recording-session:refresh-sources'),
    requestPermission: (kind: RecordingSourceKind): Promise<RecordingSessionState> =>
      ipcRenderer.invoke('recording-session:request-permission', kind),
    openSettings: (kind: RecordingSourceKind): Promise<void> =>
      ipcRenderer.invoke('recording-session:open-settings', kind),
    selectMicrophone: (sourceId: string): Promise<RecordingSessionState> =>
      ipcRenderer.invoke('recording-session:select-microphone', sourceId),
    start: (input: StartRecordingSessionInput): Promise<RecordingSessionState> =>
      ipcRenderer.invoke('recording-session:start', input),
    pause: (): Promise<RecordingSessionState> => ipcRenderer.invoke('recording-session:pause'),
    resume: (): Promise<RecordingSessionState> => ipcRenderer.invoke('recording-session:resume'),
    cancel: (): Promise<RecordingSessionState> => ipcRenderer.invoke('recording-session:cancel'),
    stop: (): Promise<StopRecordingSessionResult> => ipcRenderer.invoke('recording-session:stop'),
    onStateChanged: (callback: (state: RecordingSessionState) => void) => {
      const handler = (_event: Electron.IpcRendererEvent, state: RecordingSessionState): void => callback(state)
      ipcRenderer.on('recording-session:state-changed', handler)
      return () => ipcRenderer.removeListener('recording-session:state-changed', handler)
    },
  },
  ...
  checkDuplicates: (urls: string[]): Promise<string[]> =>
    ipcRenderer.invoke('ingest:check-duplicates', urls),
  ...
  onTabStreamEnd: (callback: (data: { episodeId: string; tabId: string; tab?: { id: string; episode_id: string; recipe_id: string | null; tab_name: string; content: string; is_pipeline: number; position: number; generated_at: string | null; generated_model: string | null; created_at: string; updated_at: string } }) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, data: { episodeId: string; tabId: string; tab?: { id: string; episode_id: string; recipe_id: string | null; tab_name: string; content: string; is_pipeline: number; position: number; generated_at: string | null; generated_model: string | null; created_at: string; updated_at: string } }): void => callback(data)
