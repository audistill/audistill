...
describe('ContentTabStore', () => {
  ...
  describe('deleteTab', () => {
    ...
    it('removes a Pipeline-created tab from state', () => {
      useContentTabStore.setState({ tabs: mockTabs, activeTabId: 'tab-1' })

      useContentTabStore.getState().deleteTab('tab-1')

      const state = useContentTabStore.getState()
      expect(state.tabs.map((t) => t.id)).toEqual(['tab-2'])
      expect(state.activeTabId).toBe('tab-2')
      expect(mockApi.tabsDelete).toHaveBeenCalledWith('tab-1')
    })

    it('does not delete the tab that is currently streaming', () => {
      useContentTabStore.setState({ tabs: mockTabs, activeTabId: 'tab-1', streamingTabId: 'tab-1' })
      ...
      expect(state.activeTabId).toBe('tab-1')
      expect(mockApi.tabsDelete).not.toHaveBeenCalled()
    })
