...
describe('RecipeService', () => {
  ...
  describe('schema and seeding', () => {
    ...
    it('built-in recipes contain prompt content from .txt files', () => {
      ...
      expect(brief.prompt).toContain('under a minute')
      expect(brief.prompt).toContain('Match the language of the transcript')
      const detailed = recipes.find((r) => r.name === 'Detailed')!
      expect(detailed.prompt).toContain('structured reference summary')
      expect(detailed.prompt).toContain('without rereading a full long-form note')
      const full = recipes.find((r) => r.name === 'Full')!
      expect(full.prompt).toContain('stand in for consuming the original recording')
      expect(full.prompt).toContain('without padding')
    })

    it('does not duplicate the full shared formatting list in built-in prompts', () => {
      const recipes = service.getRecipes()
      for (const recipe of recipes) {
        expect(recipe.prompt).not.toContain('Mermaid')
        expect(recipe.prompt).not.toContain('GFM')
        expect(recipe.prompt).not.toContain('markdown fences')
      }
    })

    it('does not re-seed duplicate rows if built-in recipes already exist', () => {
      const service2 = new RecipeService(db, PROMPTS_DIR)
      const recipes = service2.getRecipes()
      expect(recipes).toHaveLength(3)
    })

    it('overwrites existing built-in prompt text from shipped prompt files on startup', () => {
      const brief = service.getRecipes().find((r) => r.name === 'Brief')!
      service.updateRecipe(brief.id, { prompt: 'old shipped default' })

      const service2 = new RecipeService(db, PROMPTS_DIR)
      const updatedBrief = service2.getRecipe(brief.id)!

      expect(updatedBrief.prompt).toContain('under a minute')
      expect(updatedBrief.prompt).not.toBe('old shipped default')
    })
  ...
  describe('executeRecipe', () => {
    ...
    it('builds correct messages with recipe prompt as system and transcript as user', async () => {
      ...
      try {
        ...
        expect(body.messages[1].content).toContain('under a minute')
